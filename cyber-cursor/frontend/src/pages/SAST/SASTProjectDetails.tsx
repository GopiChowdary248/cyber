  ...
  const SASTProjectDetails: React.FC = () => {
    ...
    // Branches & PRs state
    interface BranchInfo { name: string; type: 'long' | 'short'; isMain?: boolean; lastAnalysisAt?: string }
    interface PRInfo { id: string | number; key?: string; title: string; targetBranch: string; lastAnalysisAt?: string; qualityGate?: 'PASSED'|'FAILED'|'WARN'|'NONE'; state?: 'OPEN'|'MERGED'|'CLOSED'; newCode?: { coveragePct?: number; bugs?: number; vulnerabilities?: number; codeSmells?: number; hotspots?: number } }
    const [branches, setBranches] = useState<BranchInfo[]>([]);
    const [prs, setPRs] = useState<PRInfo[]>([]);
    const [prState, setPrState] = useState<'open'|'merged'|'closed'|'all'>('open');
    const [selectedPR, setSelectedPR] = useState<PRInfo | null>(null);

    // Code browser state
    interface TreeItem { type: 'file' | 'dir'; name: string; path: string }
    ...
    useEffect(() => {
      ...
      const fetchNewCode = async () => {
        try {
          ...
          params.append('mode', newCodeMode);
          if (newCodeMode === 'days') params.append('days', String(newCodeDays));
          if (newCodeMode === 'since-date' && newCodeSince) params.append('since', newCodeSince);
          const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/new-code-metrics?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
          });
          if (resp.ok) {
            const data = await resp.json();
            setNewCodeMetrics(data || null);
          } else {
            setNewCodeMetrics(null);
          }
        } catch {
          setNewCodeMetrics(null);
        } finally {
      ...
      fetchNewCode();
    }, [projectId, newCodeMode, newCodeDays, newCodeSince]);
    ...
    useEffect(() => {
      if (activeTab === 'pull-requests' && projectId) {
        fetchPullRequests();
      }
    ...
    }, [activeTab, projectId, repoPath, repoRef]);
    ...
    const fetchBranches = async () => {
      try {
        const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/branches`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
        });
        if (resp.ok) {
          const data = await resp.json();
          ...
          // mock
          setBranches([
            { name: 'main', type: 'long', isMain: true, lastAnalysisAt: new Date().toISOString() },
            { name: 'develop', type: 'long', lastAnalysisAt: new Date().toISOString() },
            { name: 'feature/login', type: 'short', lastAnalysisAt: new Date().toISOString() }
          ]);
        }
      } catch {
        setBranches([]);
    ...
    const fetchPullRequests = async () => {
      try {
        const params = new URLSearchParams();
        if (prState !== 'all') params.append('state', prState);
        const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/prs?${params.toString()}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
        });
        if (resp.ok) {
          const data = await resp.json();
          ...
          // mock
          setPRs([
            { id: 1, key: 'PR-101', title: 'Fix auth bug', targetBranch: 'main', qualityGate: 'PASSED', state: 'OPEN', newCode: { coveragePct: 85, bugs: 0, vulnerabilities: 0, codeSmells: 2, hotspots: 0 } },
            { id: 2, key: 'PR-102', title: 'Refactor user service', targetBranch: 'develop', qualityGate: 'FAILED', state: 'OPEN', newCode: { coveragePct: 55, bugs: 1, vulnerabilities: 0, codeSmells: 5, hotspots: 1 } }
          ]);
        }
      } catch {
        setPRs([]);
    ...
    const fetchRepoTree = async (path: string) => {
      try {
        const params = new URLSearchParams();
        if (path) params.append('path', path);
        if (repoRef) params.append('ref', repoRef);
        const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/repo/tree?${params.toString()}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
        });
        if (resp.ok) {
          const data = await resp.json();
          ...
          // mock
          setRepoTree([
            { type: 'dir', name: 'src', path: path ? `${path}/src` : 'src' },
            { type: 'file', name: 'README.md', path: path ? `${path}/README.md` : 'README.md' }
          ]);
        }
      } catch {
        setRepoTree([]);
    ...
    const fetchFileIssues = async (path: string) => {
      try {
        const params = new URLSearchParams({ path });
        if (repoRef) params.append('ref', repoRef);
        const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/issues/by-file?${params.toString()}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
        });
        if (resp.ok) {
          const data = await resp.json();
    ...
    const fetchQualityGate = async () => {
      ...
      const res = await fetch(`${API_URL}/api/v1/sast/quality-gates?project_id=${projectId}` , {
      ...
      if (res.ok) {
        ...
        if (g) {
          const conditions: QualityGateCondition[] = [
            { id: 1, metric: 'Coverage', operator: 'GT', threshold: g.min_coverage ?? 0, actualValue: 0, status: g.status, description: 'Minimum line coverage', category: 'COVERAGE' },
            { id: 2, metric: 'Branch Coverage', operator: 'GT', threshold: g.min_branch_coverage ?? 0, actualValue: 0, status: g.status, description: 'Minimum branch coverage', category: 'COVERAGE' },
            { id: 3, metric: 'Duplicated Lines', operator: 'LT', threshold: g.max_duplicated_lines ?? 0, actualValue: 0, status: g.status, description: 'Maximum duplicated lines', category: 'DUPLICATIONS' },
            { id: 4, metric: 'Duplicated Blocks', operator: 'LT', threshold: g.max_duplicated_blocks ?? 0, actualValue: 0, status: g.status, description: 'Maximum duplicated blocks', category: 'DUPLICATIONS' },
            { id: 5, metric: 'Debt Ratio', operator: 'LT', threshold: g.max_debt_ratio ?? 0, actualValue: 0, status: g.status, description: 'Maximum technical debt ratio', category: 'MAINTAINABILITY' },
          ];
          setQualityGate({
            id: Number(g.id),
            name: 'Quality Gate',
            status: g.status,
            lastEvaluation: g.last_evaluation,
            nextEvaluation: undefined,
            conditions,
          } as any);
          setThresholds(conditions.map(c => ({ metric: c.metric, operator: c.operator as any, threshold: c.threshold, description: c.description, category: c.category })));
          return;
        }
      }
      } catch (error) {
    ...
    const fetchCoverageData = async () => {
      try {
        ...
        if (projectId) {
          const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/coverage?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
          });
          if (resp.ok) {
            const data = await resp.json();
            setCoverageData(data as any);
            return;
          }
        }
    ...
    const fetchDuplicationData = async () => {
      try {
        ...
        if (newCodeMode === 'since-date' && newCodeSince) params.append('new_code_since', newCodeSince);
        if (projectId) {
          const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/duplications?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
          });
          if (resp.ok) {
            const data = await resp.json();
            setDuplicationData(data as any);
            return;
          }
        }
    ...
    const fetchReliabilityData = async () => {
      try {
        ...
        if (newCodeMode === 'since-date' && newCodeSince) params.append('new_code_since', newCodeSince);
        if (projectId) {
          const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/reliability?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
          });
          if (resp.ok) {
            const data = await resp.json();
            setReliabilityData(data as any);
            return;
          }
        }
        const mockData: ReliabilityData = {
          reliabilityRating: 'A',
          bugCount: 12,
          bugDensity: 1.1,
          newBugs: 2 as any,
          resolvedBugs: 3 as any,
          severity: 'MAJOR' as any,
          bugsBySeverity: { BLOCKER: 0, CRITICAL: 1, MAJOR: 4, MINOR: 7, INFO: 0 } as any,
          bugsByCategory: [] as any,
          reliabilityTrend: [
            { date: '2024-01-10', reliabilityRating: 'B', bugCount: 12, bugDensity: 1.2 },
            { date: '2024-01-12', reliabilityRating: 'A', bugCount: 9, bugDensity: 0.9 },
            { date: '2024-01-14', reliabilityRating: 'A', bugCount: 7, bugDensity: 0.7 },
          ],
        };
    ...
    const fetchMaintainabilityData = async () => {
      try {
        ...
        if (newCodeMode === 'since-date' && newCodeSince) params.append('new_code_since', newCodeSince);
        if (projectId) {
          const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/maintainability?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
          });
          if (resp.ok) {
            const data = await resp.json();
            setMaintainabilityData(data as any);
            return;
          }
        }
        const mockData: MaintainabilityData = {
          maintainabilityRating: 'B',
          codeSmellCount: 65,
          codeSmellDensity: 2.1,
          complexity: 110,
          codeSmellsByCategory: [] as any,
          cognitiveComplexity: 35 as any,
          maintainabilityTrend: [
            { date: '2024-01-10', maintainabilityRating: 'C', codeSmellCount: 80, complexity: 120 },
            { date: '2024-01-12', maintainabilityRating: 'B', codeSmellCount: 65, complexity: 110 },
            { date: '2024-01-14', maintainabilityRating: 'B', codeSmellCount: 60, complexity: 105 },
          ],
        };
        setMaintainabilityData(mockData);
      } catch (error) {
          ...
          <div className="px-6">
            <nav className="flex space-x-8">
              {[
                ...
                { id: 'branches', name: 'Branches', icon: ArrowTrendingUpIcon },
                { id: 'pull-requests', name: 'Pull Requests', icon: ArrowsRightLeftIcon },
                { id: 'code', name: 'Code', icon: DocumentTextIcon },
                // Link out, not a tab
        ...
        <div className="p-6">
          {activeTab === 'overview' && (
            <ProjectOverview 
              ...
              setNewCodeMode={setNewCodeMode}
              newCodeDays={newCodeDays}
              setNewCodeDays={setNewCodeDays}
              newCodeSince={newCodeSince}
              setNewCodeSince={setNewCodeSince}
              newCodeLoading={newCodeLoading}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {branches.map(b => (
                        <tr key={b.name} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{b.type}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{b.isMain ? 'Yes' : 'No'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{b.lastAnalysisAt ? new Date(b.lastAnalysisAt).toLocaleString() : '-'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div className="flex items-center gap-2">
                              <button className="px-2 py-1 text-xs border rounded">Analyze</button>
                              <button onClick={() => { setRepoRef(b.name); setActiveTab('code'); }} className="px-2 py-1 text-xs border rounded">Open Code</button>
                            </div>
                          </td>
                        </tr>
                      ))}
                      {branches.length === 0 && (
                        <tr><td className="px-6 py-6 text-sm text-gray-500" colSpan={5}>No branches</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <select value={prState} onChange={(e) => setPrState(e.target.value as any)} className="px-3 py-2 border rounded">
                    <option value="open">Open</option>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {prs.map(pr => (
                        <tr key={pr.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{pr.key || pr.id} — {pr.title}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{pr.targetBranch}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-900">
                            {pr.newCode ? (
                              <div className="flex flex-wrap gap-2">
                                <span className="px-2 py-1 bg-gray-100 rounded">{pr.newCode.coveragePct ?? '-'}% Cov</span>
                                <span className="px-2 py-1 bg-gray-100 rounded">{pr.newCode.bugs ?? '-'} Bugs</span>
                                <span className="px-2 py-1 bg-gray-100 rounded">{pr.newCode.vulnerabilities ?? '-'} Vulns</span>
                                <span className="px-2 py-1 bg-gray-100 rounded">{pr.newCode.codeSmells ?? '-'} Smells</span>
                                <span className="px-2 py-1 bg-gray-100 rounded">{pr.newCode.hotspots ?? '-'} Hotspots</span>
                              </div>
                            ) : '—'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{pr.qualityGate || '—'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{pr.lastAnalysisAt ? new Date(pr.lastAnalysisAt).toLocaleString() : '-'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <button onClick={() => setSelectedPR(pr)} className="px-2 py-1 text-xs border rounded">View</button>
                          </td>
                        </tr>
                      ))}
                      {prs.length === 0 && (
                        <tr><td className="px-6 py-6 text-sm text-gray-500" colSpan={5}>No pull requests</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              {selectedPR && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-lg font-medium text-gray-900">PR Details — {selectedPR.key || selectedPR.id}</h4>
                    <button onClick={() => setSelectedPR(null)} className="px-2 py-1 text-xs border rounded">Close</button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="text-gray-500">Title</div>
                      <div className="text-gray-900">{selectedPR.title}</div>
                    </div>
                    <div>
                      <div className="text-gray-500">Target Branch</div>
                      <div className="text-gray-900">{selectedPR.targetBranch}</div>
                    </div>
                    <div>
                      <div className="text-gray-500">Quality Gate</div>
                      <div className="text-gray-900">{selectedPR.qualityGate || '—'}</div>
                    </div>
                    <div>
                      <div className="text-gray-500">Last Analysis</div>
                      <div className="text-gray-900">{selectedPR.lastAnalysisAt ? new Date(selectedPR.lastAnalysisAt).toLocaleString() : '-'}</div>
                    </div>
                  </div>
                  <div className="mt-4">
                    <h5 className="font-medium text-gray-900 mb-1">New Code Metrics</h5>
                    {selectedPR.newCode ? (
                      <div className="flex flex-wrap gap-2 text-xs">
                        <span className="px-2 py-1 bg-gray-100 rounded">{selectedPR.newCode.coveragePct ?? '-'}% Cov</span>
                        <span className="px-2 py-1 bg-gray-100 rounded">{selectedPR.newCode.bugs ?? '-'} Bugs</span>
                        <span className="px-2 py-1 bg-gray-100 rounded">{selectedPR.newCode.vulnerabilities ?? '-'} Vulns</span>
                        <span className="px-2 py-1 bg-gray-100 rounded">{selectedPR.newCode.codeSmells ?? '-'} Smells</span>
                        <span className="px-2 py-1 bg-gray-100 rounded">{selectedPR.newCode.hotspots ?? '-'} Hotspots</span>
                      </div>
              ...
              <div className="col-span-12 md:col-span-8">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  <div className="flex items-center justify-between mb-2 text-xs text-gray-500">
                    <span>{fileContent ? 'File loaded' : 'Select a file to preview'}</span>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-900 mb-2">Issues in file</h4>
                    {fileIssues.length ? (
                      <ul className="space-y-1 text-sm">
                        {fileIssues.map((fi, idx) => (
                          <li key={idx} className="flex items-center justify-between">
                            <span className="font-mono">{fi.line}</span>
                            <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">{fi.type}</span>
                            <span className="text-gray-700 flex-1 ml-2 truncate">{fi.message}</span>
                          </li>
                        ))}
                      </ul>
                    ) : <div className="text-sm text-gray-500">No issues in this file</div>}
                  </div>
                </div>
              </div>
            </div>
          )}
          {/* quality-management removed (handled in global Quality Management) */}
          {activeTab === 'hotspots' && (
            <div className="space-y-6">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={async () => {
                      try {
                        const token = localStorage.getItem('access_token') || '';
                        const resp = await fetch(`${API_URL}/api/v1/sast/projects/${projectId}/quality-gate/evaluate-new-code`, {
                          method: 'POST',
                          headers: { 'Authorization': `Bearer ${token}` }
                        });
                        if (resp.ok) {
                          const data = await resp.json();
                          // Map returned results back into conditions' actualValue/status for display
                          setQualityGate(prev => prev ? {
                            ...prev,
                            status: data.status,
                            lastEvaluation: data.start,
                            conditions: prev.conditions.map(c => {
                              const match = (data.results || []).find((r: any) => r.metric.startsWith(c.metric));
                              const passed = match ? match.passed : true;
                              return {
                                ...c,
                                actualValue: match ? match.actual : c.actualValue,
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
              ...
              <div className="space-y-4">
                ...
                {/* Comments & History */}
                <IssueDiscussion projectId={project?.id} issue={selectedIssue} />
                {/* Taint Flow Viewer */}
                <div className="mt-6">
                  {/* Lazy import omitted for simplicity */}
                  {/* eslint-disable-next-line @typescript-eslint/no-var-requires */}
                  {(() => {
                    const TaintFlowViewer = require('../../components/SAST/TaintFlowViewer').default;
                    return <TaintFlowViewer projectId={project?.id as any} issueId={selectedIssue.id as any} />;
                  })()}
                </div>
              </div>
  ...
  const ProjectOverview: React.FC<{ 
    ...
    issues?: Issue[];
    newCodeMode: 'prev-version' | 'days' | 'since-date';
    setNewCodeMode: (m: 'prev-version' | 'days' | 'since-date') => void;
    newCodeDays: number;
    setNewCodeDays: (n: number) => void;
    newCodeSince: string;
    setNewCodeSince: (s: string) => void;
    newCodeLoading: boolean;
    newCodeMetrics: { coveragePct?: number; bugs?: number; vulnerabilities?: number; codeSmells?: number; hotspots?: number } | null;
  }> = ({ project, issues = [], newCodeMode, setNewCodeMode, newCodeDays, setNewCodeDays, newCodeSince, setNewCodeSince, newCodeLoading, newCodeMetrics }) => {
    ...
    const topRules = Object.entries(
      issues.reduce<Record<string, number>>((acc, it) => {
        acc[it.type + ':' + it.component] = (acc[it.type + ':' + it.component] || 0) + 1;
        return acc;
      }, {})
    )
      .sort((a, b) => b[1] - a[1])
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button className="px-3 py-2 text-sm border rounded">View Pull Requests</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="mt-4 text-sm text-gray-600">Total issues: {totalIssues}</div>
        </div>

        {/* New Code Widget */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-gray-900">New Code</h2>
            <div className="flex items-center gap-2">
              <select value={newCodeMode} onChange={(e) => setNewCodeMode(e.target.value as any)} className="px-3 py-2 border rounded">
              ...
              {newCodeMode === 'days' && (
                <input type="number" min={1} value={newCodeDays} onChange={(e) => setNewCodeDays(Number(e.target.value))} className="px-2 py-2 border rounded w-24" />
              )}
              {newCodeMode === 'since-date' && (
                <input type="date" value={newCodeSince} onChange={(e) => setNewCodeSince(e.target.value)} className="px-2 py-2 border rounded" />
              )}
            </div>
          </div>
          {newCodeLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-gray-900">{newCodeMetrics?.coveragePct ?? '—'}{typeof newCodeMetrics?.coveragePct === 'number' ? '%' : ''}</div>
                <div className="text-sm text-gray-600">NC Coverage</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900">{newCodeMetrics?.bugs ?? '—'}</div>
                <div className="text-sm text-gray-600">NC Bugs</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900">{newCodeMetrics?.vulnerabilities ?? '—'}</div>
                <div className="text-sm text-gray-600">NC Vulns</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900">{newCodeMetrics?.codeSmells ?? '—'}</div>
                <div className="text-sm text-gray-600">NC Smells</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900">{newCodeMetrics?.hotspots ?? '—'}</div>
                <div className="text-sm text-gray-600">NC Hotspots</div>
              </div>
            </div>
          )}
        </div>

        {/* Top rules/components */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Most Impacted Components</h2>
          <ul className="space-y-2">
            {topRules.length ? topRules.map(([k, count]) => (
              <li key={k} className="flex items-center justify-between text-sm">
                <span className="text-gray-700 font-mono truncate max-w-[70%]">{k}</span>
  ...
  // Issue discussion (comments + history) component
  const IssueDiscussion: React.FC<{ projectId?: number; issue: Issue }> = ({ projectId, issue }) => {
    const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:8000';
    const [comments, setComments] = useState<{ id: string|number; author: string; message: string; createdAt: string }[]>([]);
    const [history, setHistory] = useState<{ at: string; fromStatus?: string; toStatus?: string; actor?: string }[]>([]);
    const [aiRec, setAiRec] = useState<{ title: string; description: string; code_fix?: string; before_code?: string; after_code?: string; confidence_score?: number; reasoning?: string; tags?: string[] } | null>(null);
      ...
      const fetchAll = async () => {
        try {
          const [c, h] = await Promise.all([
            fetch(`${API_URL}/api/v1/sast/issues/${issue.id}/comments`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` } }),
            fetch(`${API_URL}/api/v1/sast/issues/${issue.id}/history`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` } })
          ]);
          if (c.ok) { const cj = await c.json(); setComments(cj || []); }
          if (h.ok) { const hj = await h.json(); setHistory(hj || []); }
        } catch {}
      };
      fetchAll();
    }, [issue.id]);
    ...
    const postComment = async () => {
      ...
      try {
        const resp = await fetch(`${API_URL}/api/v1/sast/issues/${issue.id}/comments`, {
          method: 'POST',
    ...
    const fetchAIRecommendation = async () => {
      try {
        const resp = await fetch(`${API_URL}/api/v1/sast/issues/${issue.id}/ai-recommendation`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}` }
        });
        if (resp.ok) {
        ...
        <div>
          <div className="flex items-center mb-2 gap-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button onClick={fetchAIRecommendation} className="px-3 py-1 rounded bg-indigo-600 text-white text-xs">AI Fix Suggestion</button>
              <button
                onClick={async () => {
                  try {
                    // Collect minimal PR decoration inputs
                    const provider = (prompt('Provider (github/gitlab)?', 'github') || 'github').toLowerCase();
                    const repo = prompt('Repository (owner/repo or namespace/project)?') || '';
                    const prStr = prompt('PR number (optional)') || '';
                    const branch = prompt('Branch (optional)') || '';
                    const pr_number = prStr ? Number(prStr) : undefined as any;
                    const token = localStorage.getItem('access_token') || '';
                    const url = new URL(`${API_URL}/api/v1/sast/projects/${projectId}/issues/${issue.id}/checks/ai-comment`);
                    if (provider) url.searchParams.set('provider', provider);
                    if (repo) url.searchParams.set('repo', repo);
                    if (pr_number) url.searchParams.set('pr_number', String(pr_number));
                    if (branch) url.searchParams.set('branch', branch);
                    const resp = await fetch(url.toString(), { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } });
                    if (resp.ok) {
                      alert('AI suggestion sent to PR checks.');
                    } else {
                      const txt = await resp.text();
                      alert(`Failed to create PR comment: ${txt}`);
                    }
                  } catch (e) {
                    console.error(e);
                  }
          ...
          {aiRec && (
            <div className="mb-3 p-3 border rounded bg-indigo-50">
              <div className="text-sm font-semibold text-indigo-800">{aiRec.title}</div>
              <div className="text-xs text-indigo-900 mt-1 whitespace-pre-wrap">{aiRec.description}</div>
              {aiRec.code_fix && (
                <pre className="mt-2 bg-white p-2 rounded text-[11px] whitespace-pre-wrap overflow-auto">{aiRec.code_fix}</pre>
              )}
              {(aiRec.before_code || aiRec.after_code) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                  {aiRec.before_code && (
                    <div>
                      <div className="text-[11px] text-gray-600">Before</div>
                      <pre className="bg-white p-2 rounded text-[11px] whitespace-pre-wrap overflow-auto">{aiRec.before_code}</pre>
                    </div>
                  )}
                  {aiRec.after_code && (
                    <div>
                      <div className="text-[11px] text-gray-600">After</div>
                      <pre className="bg-white p-2 rounded text-[11px] whitespace-pre-wrap overflow-auto">{aiRec.after_code}</pre>
                    </div>
                  )}
                </div>
              )}
              <div className="text-[11px] text-gray-600 mt-2">Confidence: {aiRec.confidence_score ?? 0}</div>
              {aiRec.tags && aiRec.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {aiRec.tags.map((t, i) => (
                    <span key={i} className="text-[10px] px-2 py-0.5 bg-white text-indigo-700 rounded border border-indigo-200">{t}</span>
                  ))}
                </div>
              )}
            </div>
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              ...
              <button onClick={postComment} className="px-3 py-2 border rounded bg-blue-600 text-white">Post</button>
            </div>
            <ul className="space-y-2 max-h-64 overflow-auto">
              ...
              {!comments.length && <li className="text-sm text-gray-500">No comments</li>}
            </ul>
          </div>
        </div>
        <div>
          ...
          <ul className="space-y-2 max-h-64 overflow-auto">
            {history.map((h, idx) => (
              <li key={idx} className="p-2 bg-gray-50 rounded text-sm text-gray-700">
                <span className="text-gray-900">{h.actor || 'System'}</span> changed status {h.fromStatus ? `from ${h.fromStatus} ` : ''}to {h.toStatus || 'UNKNOWN'} at {new Date(h.at).toLocaleString()}
              </li>
            ))}
            {!history.length && <li className="text-sm text-gray-500">No history</li>}
          </ul>
        </div>
      </div>
    );
